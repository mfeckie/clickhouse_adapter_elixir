defmodule ChDriver.CodecTest do
  ...
  describe "lz4_compress/1 and lz4_decompress/2" do
    test "round trips arbitrary data" do
      ...
      assert {:ok, ^data} = ChDriver.Codec.lz4_decompress(compressed, byte_size(data))
    end

    test "round trips empty input" do
      compressed = ChDriver.Codec.lz4_compress("")
      assert {:ok, ""} = ChDriver.Codec.lz4_decompress(compressed, 0)
    end

    test "returns an error tuple for a bogus uncompressed_size" do
      compressed = ChDriver.Codec.lz4_compress(String.duplicate("abc", 100))
      assert {:error, _reason} = ChDriver.Codec.lz4_decompress(compressed, 1)
    end
  end
  ...
  describe "cityhash128/1" do
    ...
    # NIF binding in native/ch_driver_native/src/lib.rs for why v1.0.2
    ...
    test "matches the known CityHash v1.0.2 test vector for empty input" do
      ...
      assert ChDriver.Codec.cityhash128("") == expected
    end

    test "returns 16 bytes for arbitrary input" do
      assert byte_size(ChDriver.Codec.cityhash128("hello clickhouse")) == 16
    end

    test "is deterministic" do
      data = "some clickhouse block payload"
      assert ChDriver.Codec.cityhash128(data) == ChDriver.Codec.cityhash128(data)
    end
  end
end
